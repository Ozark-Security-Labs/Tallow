...
vi.stubGlobal('fetch', vi.fn(async () => new Response(JSON.stringify({ user: { id: 'u1', email: 'analyst@example.com', roles: ['analyst'], status: 'active' }, capabilities: ['findings:triage'] }), { status: 200 })));

describe('AlertDetail', () => {
  it('shows alert evidence links and triage action area', async () => {
    render(<AuthProvider><AlertDetail /></AuthProvider>);
