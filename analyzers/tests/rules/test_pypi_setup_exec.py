...
def test_safe_backend_with_inline_comment_does_not_emit(tmp_path: Path):
    (tmp_path / "manifest.json").write_text('{"files":[]}', encoding="utf-8")
    (tmp_path / "pyproject.toml").write_text(
        '[build-system]\nbuild-backend = "setuptools.build_meta" # safe fixture\n',
        encoding="utf-8",
    )
    assert _run(tmp_path) == []


def test_same_line_setup_py_exec_sinks_have_distinct_finding_ids(tmp_path: Path):
    (tmp_path / "manifest.json").write_text('{"files":[]}', encoding="utf-8")
    (tmp_path / "setup.py").write_text(
        "import os, subprocess\nos.system('echo synthetic'); subprocess.run(['echo'])\n",
        encoding="utf-8",
    )
    findings = [
